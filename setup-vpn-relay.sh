...
# ============================================================================
# XRAY RELAY MANAGER - Production Ready v6.2.3 (Final)
# Архитектура: Вариант B (Namespace + veth + socat)
# ИСПРАВЛЕНО v6.2.3: Проверка занятости порта 443 перед запуском socat
# ============================================================================

set -euo pipefail

readonly VERSION="6.2.3"
...
section "Шаг 15: Veth-пара и socat (ИСПРАВЛЕНО v6.2.3)"
...
ExecStartPre=/bin/bash -c 'if ss -tlnp | grep -q ":443"; then echo "ERROR: Port 443 already in use"; ss -tlnp | grep :443; exit 1; fi'
...
Type=simple
ExecStartPre=/bin/sleep 10
ExecStartPre=/bin/bash -c 'if ss -ulnp | grep -q ":8443"; then echo "ERROR: Port 8443 already in use"; ss -ulnp | grep :8443; exit 1; fi'
...
log "✓ Socat сервисы (Type=simple, ExecStartPre=sleep 10, проверка портов)"

section "Шаг 16: Запуск всех сервисов (ИСПРАВЛЕНО v6.2.3)"
...
cmd_help() {
    echo -e "${CYAN}xray-admin v6.2.3 - VPN Relay Manager${NC}"
    echo ""
    echo "Команды:"
...
echo -e "${GREEN}╔══════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  VPN Relay v6.2.3 (Namespace + veth + socat)       ║${NC}"
echo -e "${GREEN}╚══════════════════════════════════════════════════════╝${NC}"
echo ""
...
print(f"✅ Скрипт v6.2.3 создан: /root/setup-vpn-relay.sh")
print(f"📊 Размер: {os.path.getsize('/root/setup-vpn-relay.sh')} байт")
print(f"📝 Строк: {script.count(chr(10))}")
print(f"\n🔧 Ключевые исправления в v6.2.3:")
print(f"  ✓ ИСПРАВЛЕНО: Проверка занятости порта 443/8443 перед запуском socat")
print(f"  ✓ ИСПРАВЛЕНО: ExecStartPre для проверки портов в systemd")
print(f"  ✓ ИСПРАВЛЕНО: Подробная диагностика при ошибке")
print(f"  ✓ ИСПРАВЛЕНО: Проверка veth-пары перед запуском socat")
PYEOF
