...
cleanup_netns() {
    ...
    if ip netns list 2>/dev/null | grep -q "^$NAMESPACE"; then
        # Удаляем awg0 внутри namespace (если существует)
        ...
        # Удаляем veth-ns внутри namespace (если существует)
